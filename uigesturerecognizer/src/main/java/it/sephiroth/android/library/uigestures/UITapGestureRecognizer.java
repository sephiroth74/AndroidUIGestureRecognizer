...
public final class UITapGestureRecognizer extends UIGestureRecognizer implements UIDiscreteGestureRecognizer {
    ...
    protected void handleMessage(final Message msg) {
        switch (msg.what) {
            case MESSAGE_RESET:
                logger.info("handleMessage(MESSAGE_RESET)");
                handleReset();
                break;

            case MESSAGE_FAILED:
                logger.info("handleMessage(MESSAGE_FAILED)");
                handleFailed();
                break;

            case MESSAGE_POINTER_UP:
                logger.info("handleMessage(MESSAGE_POINTER_UP)");
                mNumTouches = msg.arg1;
                break;

            case MESSAGE_LONG_PRESS:
                logger.info("handleMessage(MESSAGE_LONG_PRESS)");
                handleFailed();
                break;
    ...
    public void onStateChanged(@NonNull final UIGestureRecognizer recognizer) {
        logger.verbose("onStateChanged(%s): %s", recognizer, recognizer.getState());
        logger.verbose("this.state: %s", getState());
        logger.verbose("mStarted: %s", mStarted);
